...
Mapping is the process of establishing an intnal representation of environments. Metric-semantic maps include human-readable information (*i.e.,* map elements are labelled), which provideds a better understanding of environments, in constrast to the pure metric map which store geometry of a scene and are usually defined by positions of landmarks (point cloud map), distance to obstacles (distance field), or binary values to indicate free and occupied space (occupancy map). Recent works in scene abstraction (*e.g.,* [Hydra](https://github.com/MIT-SPARK/Hydra)), long-term map update (*e.g.,* [Panoptic Mapping](https://github.com/ethz-asl/panoptic_mapping)), exploration (e.g., [Semantic Exploration](https://devendrachaplot.github.io/projects/semantic-exploration), and grasping (*e.g.,* [GOAT](https://theophilegervet.github.io/projects/goat)) have demonstrated the potential of semantic maps, boosting the development of embodied intelligence. 
...
1. **State Estimator** modifies the R3LIVE system (an Extended Kalman Filter-based LiDAR-visual-inertial odometry) to estimate real-time sensors’ poses with a local and sparse color point cloud. 
...
* **FusionPortable**: campus-type scenarios with estimated camera semantics (only applied to the Campus_Road sequence)
